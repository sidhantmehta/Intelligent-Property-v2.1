...
class _GetDataFromURL(object):
    ...
    def get_page(self, request_content):
        ...
        xp_reduce_added_dates = """//span[@class='propertyCard-branchSummary-addedOrReduced']/text()"""
        ...
        reduce_added_dates = tree.xpath(xp_reduce_added_dates)
        desciptions = tree.xpath(xp_descriptions)

        # Store the data in a Pandas DataFrame:
        data = [price_pcm, titles, addresses, weblinks, agent_urls, reduce_added_dates, desciptions]
        temp_df = pd.DataFrame(data)
        temp_df = temp_df.transpose()
        temp_df.columns = ["price", "type", "address", "url", "agent_url", "reduce_or_added_date", "descriptions"]
